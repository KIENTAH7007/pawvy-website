...
async function request(path, { method = 'GET', body, auth = false, cache, next } = {}) {
...
export const shopApi = {
  ...
  // Same real, already-filtered endpoint as brands() above, but cached
  // for a minute instead of no-store (Aug 2026) — used by the root
  // layout's Nav, which wraps every single page on the site. Hiding or
  // showing a brand via Pawvy App is a rare admin action, not something
  // that needs to propagate to the nav dropdown within milliseconds;
  // using the same no-store fetch here would force every page that
  // uses this layout (login, signup, account, blog, etc — not just the
  // shop/brand pages that already need always-fresh data) to stop
  // being statically generated at all. brands() itself stays no-store
  // for callers that genuinely need instant freshness (the brand page's
  // own hidden-brand 404 check, the sitemap).
  brandsForNav: () => request('/api/shop/brands', { next: { revalidate: 60 } }),
  topSellers: (limit = 8) => request(`/api/shop/top-sellers?limit=${limit}`, { cache: 'no-store' }),
  pawvyPicks: () => request('/api/shop/pawvy-picks', { cache: 'no-store' }),
  testimonials: (need) => request(`/api/shop/testimonials?need=${encodeURIComponent(need)}`, { cache: 'no-store' }),
  bundles: (need) => request(`/api/shop/bundles${need ? `?need=${encodeURIComponent(need)}` : ''}`, { cache: 'no-store' }),
  bundle: (id) => request(`/api/shop/bundles/${id}`, { cache: 'no-store' }),
};

export const waitlistApi = {
  join: (product_id, email) => request('/api/waitlist', { method: 'POST', body: { product_id, email } }),
};
