...
export const BRAND_CONTENT = {
  ...
  'Salmoil': {
    tagline: 'Formulated by pet nutritionist',
    description: "Salmoil's carefully formulated recipes combine salmon fish oil with functional ingredients to deliver targeted health benefits, making it a trusted choice for pet owners seeking a convenient and effective daily wellness supplement.",
    exclusive: true,
    ...
    faqs: [
      ['Does Salmoil need to be refrigerated?', "No, Salmoil does not require refrigeration. Salmoil is housed in an aluminium bottle specifically designed to shield the oil from heat, light, and oxidation — the primary causes of fish oil degradation. This advanced packaging preserves the oil's freshness, potency, and quality without the need for cold storage. Simply store your Salmoil in a cool, dry place with the cap securely tightened after each use."],
      ['Can cats have Salmoil too?', 'Salmoil is safe and beneficial for both dog and cat. Check the feeding instructions for specific dosage.'],
      ['How much Salmoil should I give my dog/cat?', `
    ...
    deepDive: {
      ...
      selector: {
        ...
        sub: "Every Salmoil recipe starts from the same Norwegian salmon oil Omega-3 base \u2014 what changes is the functional ingredients added on top for a specific benefit. Click a recipe (or hover, on desktop) to see what that is.",
        items: [
  ...
  'GiGwi': {
    ...
    exclusive: false,
    faqs: [
      ['My dog does not play with toy. Why?', "Some dogs ignore toys because they haven't found the right type yet — try experimenting with different textures, sounds, and styles like squeaky, tug, or puzzle toys. Making the toy move and playing with your dog (rather than just handing it over) is often the key to building interest. If your dog used to play but suddenly stopped, it's worth a vet visit, as pain or illness can reduce play drive."],
      ['Are GiGwi toys suitable for aggressive chewers?', 'Heavy chewers can opt for toys like the GiGwi Eco Wooden Antler, Pop Pals Ball or the S-Bone, which are created especially for tougher play. However, gentle chewers may prefer softer plushies that GiGwi offers.'],
      ['How do I keep my pet interested in their toys long-term?', 'Rotate toys regularly, leaving only one or two out at a time. Novelty plays a big role in keeping pets engaged — swapping toys every few days makes them feel new again.'],
    ],
    ...
    deepDive: {
      intro: {
        ...
        heading: 'Toys built to survive playtime',
        ...
        values: [
          { title: 'Safety certified', body: 'Quality is ensured before the toys ever reach a shelf.' },
          { title: 'Multiple materials, one toy', body: 'Different textures in a single toy encourage longer, more engaged playtime.' },
          { title: 'Made to be loved', body: 'Vibrant colors and lifelike designs \u2014 GiGwi toys are made to be picked first out of the pile.' },
        ],
      },
      categoryIntro: {
        ...
        // iconScale (new): Plush and Enrichment's source PNGs have more
        // built-in transparent padding around the toy than Ball/Chew's do
        // — the CSS container is identical for all four (88px, image at
        // 78% fill), so the size difference Janice flagged isn't a CSS
        // sizing issue, it's baked into these two image files themselves.
        // Compensating with a visual scale-up on just these two rather
        // than re-exporting the source images. 1.35 is an estimate, not
        // measured against the actual files pixel-by-pixel — worth a
        // visual check after deploy and an easy number to tweak here if
        ...
        items: [
          ...
          { title: 'Plush', color: '#F9DAD9', image: '/brand-features/gigwi/icon-plush.png', iconScale: 1.35, body: "Comfort and security \u2014 a soft companion for downtime, crate training, or a puppy's first night home." },
          { title: 'Enrichment', color: '#7B9FE0', image: '/brand-features/gigwi/icon-enrichment.png', iconScale: 1.35, body: 'Mental stimulation through puzzles and treat-dispensing play \u2014 reduces boredom and destructive behavior.' },
        ],
      },
      browser: {
        eyebrow: 'The full range',
        heading: 'Shop by category',
        sub: 'Fun time, made enriching.',
      tabs: [
        { id: "ball", label: "Ball", cards: [
          { type: 'single', name: "Neon Glow Ball", skuPrefix: "3019", featured: false },
          { type: 'group', name: "Orange Tennis Ball", featured: true, variants: [{ label: "S (D - 4.8CM)", skuPrefix: "4162" }, { label: "M (D - 6.4CM)", skuPrefix: "4165" }] },
          ...
          // New Aug 2026 batch (11 SKUs) — added under Archived in Pawvy
          // App since not yet arrived in Singapore; per the "Archived/
          // removed SKUs just don't match" note above, this card stays
          // invisible until KT flips is_active on, then appears with no
          // further code change needed. featured left false — not
          // specified in KT's SKU sheet, easy one-line flip once KT
          // confirms which of these 11 should launch featured.
          { type: 'single', name: "Fox with Spiky Ball", skuPrefix: "9131", featured: false },
        ] },
        ...
        { id: "plush", label: "Plush", cards: [
          ...
          // New Aug 2026 batch (11 SKUs total, 8 land in Plush) — see the
          // matching comment on Fox with Spiky Ball in the Ball tab above;
          // same story here, all currently invisible until un-archived.
          { type: 'single', name: "Koala with Backpack", skuPrefix: "4109", featured: false },
          { type: 'single', name: "Elephant with Rope Tail", skuPrefix: "4425", featured: false },
          { type: 'single', name: "Crocodile with Rope Tail", skuPrefix: "4426", featured: false },
          { type: 'single', name: "Giraffe with No Stuffing", skuPrefix: "4464", featured: false },
          { type: 'single', name: "Skunk with Sponge Squeaker", skuPrefix: "6073", featured: false },
          { type: 'single', name: "Lion Cloth", skuPrefix: "6862", featured: false },
          { type: 'single', name: "Fox with Crinkle Paper", skuPrefix: "9134", featured: false },
          { type: 'single', name: "Frog with Crinkle Paper", skuPrefix: "9135", featured: false },
        ] },
        { id: "enrichment", label: "Enrichment", cards: [
          ...
          // New Aug 2026 batch — see matching comment on Fox with Spiky
          // Ball in the Ball tab above.
          { type: 'single', name: "Wild Hunter Green Monster", skuPrefix: "9127", featured: false },
        ] },
  ...
  'Wild Balance': {
    ...
    description: "Complete, natural meals and treats for dogs — casseroles, freshly cooked food, chews, and more. No freezer, no thawing, no hassle: just open the pack and serve.",
    exclusive: true,
    deepDive: {
      // Two intro sections before the product cards, per KT/Janice's
      // review (Aug 2026) — every other curated brand page (Puzzle
      // Feeder is closest in scope to Wild Balance) has 1-2+ intro
      // sections before its products; Wild Balance went straight into
      // cards with none. Real intro + featureSplit pattern, same as
      // Puzzle Feeder — approved via a real HTML mockup first, not
      // invented on the fly. Both images are deliberately left as
      // placeholders (image field omitted) — KT will supply real
      // photos later; just set `image: '/brand-features/wild-balance/
      // ...'` on each once they're uploaded to public/brand-features/
      // wild-balance/, no other change needed.
      intro: {
        eyebrow: 'Real Food, Real Simple',
        heading: "Cooked the way you'd cook for them yourself",
        body: "Wild Balance makes complete meals and treats using real meat and vegetables, cooked low and slow — no fillers, no shortcuts. Every recipe is formulated to FEDIAF standards, the European equivalent of AAFCO, and every product is shelf-stable: no freezer, no thawing, just open the pack and serve.",
        imageHint: 'Wild Balance lifestyle photo',
        values: [
          { title: 'No Freezer, No Thawing', body: 'Shelf-stable until opened — a real point of difference from most fresh dog food in Singapore, which typically needs freezing.' },
          { title: 'FEDIAF-Formulated', body: 'The European equivalent of AAFCO, built on the same core nutritional science and independently reviewed.' },
          { title: 'Exclusively at Pawvy', body: 'Pawvy is the exclusive distributor of Wild Balance in Singapore.' },
        ],
      },
      featureSplit: {
        eyebrow: 'Real ingredients, nothing hidden',
        heading: 'Built from real meat, real vegetables, real spices',
        body: "Every casserole and freshly cooked meal starts with a genuine protein — anchovy, salmon, pork, beef, duck, or chicken — paired with pumpkin, carrot, and spinach for natural fibre. No grains, no unnecessary additives, no fillers standing in for real food.\n\nEven the natural chews are single-ingredient, dehydrated at low temperature with nothing else added — exactly what's on the label, nothing more.",
        imageHint: 'Real ingredient photography',
      },
      // Confirmed against KT's real Products & Pricing screenshot (Aug
      // 2026) — every SKU's item_series consistently includes its
      // category name ("Wild Balance Casseroles", "Wild Balance Freshly
      // Cooked", "Wild Balance Ice Cream", "Wild Balance Natural
      // Snacks"), which is what seriesIncludes matches against below.
      // Same real, standard item/variant shape every other curated
      // brand page's fitCardGroups already uses (Lillidale, Puzzle
      // Feeder, Salmoil, East Sea Brother) — rendered through the same
      // real <FitCard> component, not a lookalike.
      //
      // Deliberately named wbGroups, not fitCardGroups: this brand needs
      // its own custom section ordering (a feeding calculator right
      // after Casseroles and again after Freshly Cooked, a chew
      // recommendation tool before Natural Chews' grid, plus the Frozen
      // Yoghurt toggle section that isn't a card grid at all) —
      // WildBalanceDeepDive.jsx reads this key and renders it manually
      // with the real FitCard component in the right places. Naming it
      // fitCardGroups instead would make BrandDeepDive.jsx's own
      // generic fitCardGroups renderer ALSO pick it up automatically,
      // duplicating every section.
      wildBalanceSections: true,
      wbGroups: [
        {
          anchor: 'casseroles',
          eyebrow: 'Homemade-Style Casseroles',
          heading: 'Cooked low and slow, ready in seconds',
          sub: 'Real meat and vegetables in their own natural sauce — no freezer, no thawing. Just open the 280g pack and serve.',
          items: [
            {
              name: 'Anchovy, Turkey & Apple Casserole', fitFor: '280g',
              variants: [{ label: '280g', seriesIncludes: 'Casseroles', variationIncludes: 'Anchovy' }],
            },
            {
              name: 'Salmon, Chicken & Pear Casserole', fitFor: '280g',
              variants: [{ label: '280g', seriesIncludes: 'Casseroles', variationIncludes: 'Salmon' }],
            },
            {
              name: 'Pork & Pumpkin Casserole', fitFor: '280g',
              variants: [{ label: '280g', seriesIncludes: 'Casseroles', variationIncludes: 'Pork' }],
            },
            {
              name: 'Beef & Pineapple Casserole', fitFor: '280g',
              variants: [{ label: '280g', seriesIncludes: 'Casseroles', variationIncludes: 'Beef' }],
            },
          ],
        },
        {
          anchor: 'freshly-cooked',
          alt: true,
          eyebrow: 'Freshly Cooked Dog Food',
          heading: 'A complete daily meal, no freezer required',
          sub: '90% real meat, cooked low and slow with pumpkin, carrot, and spinach. Shelf-stable until opened — no thawing, ever.',
          items: [
            {
              name: 'Freshly Cooked Duck', fitFor: '200g · 400g',
              variants: [
                { label: '200g', default: true, seriesIncludes: 'Freshly Cooked', variationIncludes: 'Duck 200' },
                { label: '400g', seriesIncludes: 'Freshly Cooked', variationIncludes: 'Duck 400' },
              ],
            },
            {
              name: 'Freshly Cooked Chicken', fitFor: '200g · 400g',
              variants: [
                { label: '200g', default: true, seriesIncludes: 'Freshly Cooked', variationIncludes: 'Chicken 200' },
                { label: '400g', seriesIncludes: 'Freshly Cooked', variationIncludes: 'Chicken 400' },
              ],
            },
            {
              name: 'Freshly Cooked Salmon', fitFor: '200g · 400g',
              variants: [
                { label: '200g', default: true, seriesIncludes: 'Freshly Cooked', variationIncludes: 'Salmon 200' },
                { label: '400g', seriesIncludes: 'Freshly Cooked', variationIncludes: 'Salmon 400' },
              ],
            },
            {
              // Beef only has a 200g SKU in the real catalogue right now
              // (confirmed against the screenshot — no WFBE400 exists
              // yet) — a single-variant item, same as Plaque Guard/Ear
              // Cleaner elsewhere in this file when a line has just one
              // real size.
              name: 'Freshly Cooked Beef', fitFor: '200g',
              variants: [{ label: '200g', seriesIncludes: 'Freshly Cooked', variationIncludes: 'Beef 200' }],
            },
          ],
        },
        {
          anchor: 'natural-chews',
          eyebrow: 'Natural Chews',
          heading: 'Single-ingredient, dehydrated, no additives',
          sub: 'Six real animal-part chews — softness and chew time vary by cut. Not sure which one fits your dog? Use the picker below.',
          items: [
            {
              name: 'Lamb Gullet', fitFor: 'Very gentle / new to chews', chewCardId: 'chew-card-gullet',
              variants: [{ label: 'Lamb Gullet', seriesIncludes: 'Natural Snacks', variationIncludes: 'Lamb Gullet' }],
            },
            {
              name: 'Lamb Green Tripe', fitFor: 'Light chewer', chewCardId: 'chew-card-tripe',
              variants: [{ label: 'Lamb Green Tripe', seriesIncludes: 'Natural Snacks', variationIncludes: 'Lamb Green Tripe' }],
            },
            {
              // Two real chews land in the "Moderate chewer" bucket
              // (this one and Beef Trachea below) — the recommender can
              // only scroll/highlight one representative card per
              // button (same single-target mechanism as BetterBone's
              // durability selector), so this one carries the id; Beef
              // Trachea still shows normally in the grid right next to
              // it either way.
              name: 'Lamb Trachea', fitFor: 'Moderate chewer', chewCardId: 'chew-card-trachea',
              variants: [{ label: 'Lamb Trachea', seriesIncludes: 'Natural Snacks', variationIncludes: 'Lamb Trachea' }],
            },
            {
              name: 'Beef Trachea', fitFor: 'Moderate chewer',
              variants: [{ label: 'Beef Trachea', seriesIncludes: 'Natural Snacks', variationIncludes: 'Beef Trachea' }],
            },
            {
              // Same reasoning as Lamb Trachea above — "Determined
              // chewer" also covers 2 real chews, this one carries the
              // scroll-target id.
              name: 'Goat Headskin', fitFor: 'Determined chewer', chewCardId: 'chew-card-headskin',
              variants: [{ label: 'Goat Headskin', seriesIncludes: 'Natural Snacks', variationIncludes: 'Goat Headskin' }],
            },
            {
              name: 'Camel Headskin', fitFor: 'Determined chewer',
              variants: [{ label: 'Camel Headskin', seriesIncludes: 'Natural Snacks', variationIncludes: 'Camel Headskin' }],
            },
          ],
        },
      ],
      yoghurt: {
        seriesIncludes: 'Ice Cream',
      },
    },
    faqs: [
      ['What does "formulated to FEDIAF standards" mean, and how does it compare to AAFCO?',
        "FEDIAF is the EU's standard for complete pet food — the direct European equivalent of AAFCO, the standard most Singapore shoppers are familiar with. Both trace back to the same underlying NRC nutritional science and are independently reviewed by veterinary nutrition experts — neither is a lesser regional variant of the other. Neither AAFCO nor FEDIAF is itself a government regulator; both are industry-run, science-based standards that regulators, veterinarians, and manufacturers rely on across their respective regions. The differences that exist are minor and specific — things like calcium-to-phosphorus ratio targets or vitamin D levels — not a gap in overall rigour. FEDIAF's guidelines are published openly at europeanpetfood.org if you'd like to read the source directly."],
      ['Do Wild Balance products need to be refrigerated or frozen?',
        "No — every Wild Balance product Pawvy carries is shelf-stable, no cold-chain storage required. Simply refrigerate after opening a casserole or freshly cooked pack, and use within the stated window on the pack (24 hours for casseroles, 3 days for Freshly Cooked)."],
    ],
  },
};
